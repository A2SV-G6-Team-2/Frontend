...
function NavElement({ title, icon, isActive = false, link, onClick }: { title: string, icon: string, isActive?: boolean, link: string, onClick?: () => void }) {
    if (isActive) {
        return (
            <div className="flex items-center bg-accent rounded-xl mt-3 py-2 px-5 text-white">
                <Icon src={icon} />
                <p className='ml-3 text-md font-medium'>{title}</p>
            </div>);
    } else {
        return (
            <Link 
    ...
    return (
        ...
        `}>
            ...
            <div className="lg:hidden absolute top-4 right-4">
                <button onClick={onClose} className="p-2 text-secondary hover:text-accent transition-colors cursor-pointer">
            ...
            <div className="flex items-center gap-4 bg-white p-2 mt-auto pt-4 lg:pt-0">
                ...
                <div className="relative group shrink-0">
                    <button className="text-secondary hover:text-accent cursor-pointer transition-colors p-1">
                        <Icon src="/img/icons/logout.svg" className="w-4 h-4" />
                    </button>
                    <div className="absolute bottom-full right-0 mb-2 px-2 py-1 bg-black text-white text-[10px] rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
                        Logout
                    </div>
                </div>
            </div>
